...
<div class="col-sm-6 p-r-15-sr991 p-b-5">
	<div class="how2 how2-cl2 flex-sb-c m-b-35">
		<h3 class="fs-22 cl13 tab01-title">राेचक</h3>
	</div>
		<?php 
			$interestingNews_first = new WP_Query(array(
				'posts_per_page' => 1,
				'category_name' => 'राेचक',
			));
			$interestingNews_rest = new WP_Query(array(
				'posts_per_page' => 3,
				'category_name' => 'राेचक',
				'offset' => 1,
			));
			if($interestingNews_first->have_posts()) {
				while($interestingNews_first->have_posts()) {
					$interestingNews_first->the_post(); $interestingNews_first-> gt_set_post_view(); ?>
						<!-- Main Item post -->	
						<div class="m-b-30">
		...
		?>
		<?php
			while($interestingNews_rest->have_posts()) {
				$interestingNews_rest->the_post(); $interestingNews_rest-> gt_set_post_view(); ?>
				<!-- Item post -->	
